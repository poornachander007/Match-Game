...
//-------------------------------------------------------------------------------------------------------
const TabItem = props => {
  const {details, onClickTab, activeTabId} = props
  const {tabId, displayText} = details

  const clickTab = () => {
    console.log('onclick got hit in TabItem Component........')
    onClickTab(tabId)
  }
    ...
    <li>
      <button
        ...
        className={`tab_button ${
          tabId === activeTabId ? 'activeBtnStyle' : ''
        }`}
      >
        {displayText}
      </button>
...
const ImageItem = props => {
  ...
  const onClickMatch = () => {
    console.log('onclick got hit in ImageItem Component........')
    onClickThumbnail(id)
  }

  return (
    <li className="thumbnail_container">
      <button onClick={onClickMatch} type="button">
...
class MatchGame extends Component {
  ...
  getRandImageObj = () => {
    const randIndex = Math.ceil(Math.random() * (imagesList.length - 2))
    return imagesList[randIndex]
  }

  onClickThumbnail = id => {
    console.log('onClickThumbnail  got hit.........')
    const {randImageObj} = this.state
    if (randImageObj.id === id) {
      console.log('ID matched...........')
      this.setState(preState => ({
        score: preState.score + 1,
        randImageObj: this.getRandImageObj(),
      }))
    } else {
      this.setState({isGameRunning: false})
    }
  ...
  render() {
    const {isGameRunning, randImageObj, score, tabId, seconds} = this.state
    const filteredImageList = imagesList.filter(
      eachImage => eachImage.category === tabId,
    )
    return (
      <div className="match_game_page">
        <nav className="nav_match_game">
          <img className="logo_app" alt="website logo" src={websiteLogoUrl} />
          <ul className="options_container">
            <li>
              <p className="score_para">
                Score: <span className="score_display"> {score}</span>
              </p>
            </li>
            <li>
              <img className="icon_timer" alt="timer" src={timerUrl} />
            </li>
            <li>
              <p className="score_display">
                {seconds} sec
                {/* timer_icon_and_tiem_para */}
                {/* <span className="score_display">{seconds} sec</span> */}
              </p>
            </li>
          </ul>
        </nav>
        ...
        <div className="bottom_container">
          {isGameRunning && (
            <div className="content_container">
              <img
                ...
                src={randImageObj.imageUrl}
                alt="match"
              />
              <ul className="tabsList_container">
              ...
              <ul className="ul_imageItems_container">
          ...
          {!isGameRunning && (
            <div className="gameResultContainer">
              <img className="trophy_Image" src={trophyUrl} alt="trophy" />
              <p className="yourScore_para">YOUR SCORE</p>
              <p className="display_score">{score}</p>
              <div className="button_container">
                ...
                >
                  <img
                    className="icon_play_again"
                    src={playAgainIconUrl}
                    alt="reset"
                  />
                  PLAY AGAIN
                </button>
